    ...
    for atv in atividades:
        ...
        with col2:
            if st.button("🗑️", key=f"del_{atv[0]}"):
                c.execute("DELETE FROM atividades WHERE id=?", (atv[0],))
                conn.commit()
                st.rerun()
...
# =========================
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            status = "✅ Enviado" if row["enviado"] == 1 else "⏳ Não enviado"
            st.write(f"Status: {status}")

            if row["enviado"] == 0:
                if st.button(f"📧 Enviar para {row['nome']}"):
                    caminhos = row["imagens"].split(";") if row["imagens"] else []
